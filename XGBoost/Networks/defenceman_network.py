...
def main():
    ...
    r2 = -math.inf
    best_reg = None
    for i in tqdm(range(200), desc='Generating Defencemen Model'):
        ##### Split Data #####
        data_train, data_test, points_train, points_test = train_test_split(stats, points, test_size=0.3)

        ##### Create and Train the Model #####
        reg = xgb.XGBRegressor(objective ='reg:squarederror', n_estimators=200, max_depth=4, learning_rate=0.5)
    ...
    try:
        ...
        os.remove(path / 'defenceman_model.sav')
        file = open(path / 'defenceman_model.sav', 'wb')
